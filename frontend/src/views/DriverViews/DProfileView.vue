    ...
    <div class="bg-gray-100 min-h-screen py-10 px-4 flex justify-center items-center">
      <div class="w-full max-w-4xl bg-white rounded-3xl shadow-xl p-10">
        ...
        <div class="text-center mb-10">
          <h2 class="text-4xl font-extrabold text-gray-800">Driver Profile</h2>
...
const fetchDriverProfile = async () => {
  try {
    const email = sessionStorage.getItem('driver_email')
    if (!email) {
      window.location.href = '/'
      return
    }

    const response = await api.get('/api/driver/driver-profile/', {
      params: { email }
    })

    driver.value = response.data.data
    editedName.value = driver.value.name
    profilePreview.value = driver.value.profile_photo

  } catch (error) {
    console.error('Failed to fetch driver data:', error)
    alert('Failed to load driver profile. Please try again.')
  }
}
...
const uploadImageToImgBB = async () => {
  if (!selectedImage.value) return null; // use selectedImage, not profilePhoto
...
const saveProfile = async () => {
  ...
  try {
    ...
    // Step 2: Build updatedData
    const updatedData = {
      name: editedName.value,
      profile_photo: profilePhotoUrl || driver.value.profile_photo,
    };

    // Step 3: Call backend API
    await api.put('/api/driver/driver-profile/', {
    ...updatedData,
