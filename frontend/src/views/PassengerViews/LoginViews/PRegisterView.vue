...
const isLoggedInToday = getCookie('daily_auth') === 'true';
if (isLoggedInToday) {
  const savedEmail = sessionStorage.getItem("passenger_email");
  if (savedEmail) {
  ...
  <div class="flex flex-col items-center justify-center min-h-screen bg-gray-50 px-4 py-12">
    ...
    <div class="w-full max-w-md p-8 space-y-6 bg-white shadow-xl rounded-xl">
      ...
      <p class="text-center text-sm text-gray-600">
        Enter your email to sign in or register as a Karzo Passenger.
      </p>
      ...
      <form @submit.prevent="handleRegister" class="space-y-5">
        ...
        <div>
          <label for="passenger-email" class="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
          <input
            id="passenger-email"
            type="email"
            v-model="email"
            ...
            placeholder="Enter your passenger email"
            aria-label="Passenger Email Address"
       ...
       <p class="text-center text-xs text-gray-500 pt-4 border-t border-gray-200">
         Need help? Visit our
         <a href="#" class="underline hover:text-gray-700">Passenger Support Center</a>.
      </p>
    </div>
  </div>
</template>