...
// Define event form state for passengers
const eventForm = ref({
    name: '',
    date: '',
    time: '',
...
<template>
    <div class="max-w-2xl mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4 text-gray-800">Suggest a New Event</h1>
        <div class="bg-white shadow-lg rounded-xl p-6">
            <form @submit.prevent="submitEvent" class="space-y-4">
                <!-- Event Name -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Event Name</label>
                    <input v-model="eventForm.name" type="text"
                ...
                <div class="relative">
                    ...
                    <input v-model="locationQuery" type="text" @input="fetchLocationSuggestions"
                        class="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                        required />
                    <!-- Suggestions Dropdown -->
                    <ul v-if="locationSuggestions.length"
                        class="absolute w-full bg-white border border-gray-300 rounded-lg shadow-md mt-1 z-10">
                        <li v-for="location in locationSuggestions" :key="location.display_name"
                            @click="selectLocation(location)" class="p-2 cursor-pointer hover:bg-gray-100">
